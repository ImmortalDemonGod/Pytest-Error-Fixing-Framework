...
class CLI:
    ...
    def _handle_manual_fix_choice(self, error: TestError) -> Tuple[bool, bool]:
    ...
    def _handle_ai_fix_choice(self, error: TestError) -> Tuple[bool, bool]:
        """Handle the 'y' user choice: run AI-based fix."""
        click.echo("Attempting AI-based fix...\n")
        fixed = self.run_fix_workflow(error, interactive=True)
        if fixed:
    ...
    def _process_interactive_error(self, error: TestError) -> Tuple[bool, bool]:
        """
        Handles interactive error processing logic.
        Returns (continue, fixed): continue=False means quit the loop.
        """
        choice: Optional[str] = self._prompt_for_fix(error)
        if choice == "q":
            self._handle_quit_choice(error)
            return (False, False)
        if choice == "n":
            self._handle_skip_choice(error)
            return (True, False)
        if choice == "m":
            return self._handle_manual_fix_choice(error)
        # 'y' or any unrecognised input defaults to AI fix
        return self._handle_ai_fix_choice(error)

    def _process_non_interactive_error(self, error: TestError) -> bool:
        """
        Handles non-interactive error processing logic.
        """
        result = self.run_fix_workflow(error, interactive=False)
        if result:
    ...
    def _process_all_errors(
        ...
        for i, error in enumerate(errors, 1):
            ...
            if interactive:
                should_continue, fixed = self._process_interactive_error(error)
                if not should_continue:
                    break
                if fixed:
                    success_count += 1
            else:
                if self._process_non_interactive_error(error):
                    success_count += 1
